...
from app.domain.subsidy.subsidySchema import (
    SubsidyDetailResponse,
    SubsidyFilterRequest,
    SubsidyListResponse,
)
from app.domain.subsidy.subsidyService import collect_subsidies, get_subsidies, get_subsidy_by_id

router = APIRouter()


@router.post("/recommend", response_model=SubsidyListResponse)
async def recommend_subsidies(request: SubsidyFilterRequest, db: AsyncSession = Depends(get_db)):
    ...
    return SubsidyListResponse(
        total=total, page=request.page, size=request.size, data=[dict(row._mapping) for row in rows]
    )


@router.get("/detail/{subsidy_id}", response_model=SubsidyDetailResponse)
async def get_subsidy_detail(subsidy_id: UUID, db: AsyncSession = Depends(get_db)):
