    ...
    <div className="space-y-8">
      ...
      {form.contractType === 'MINOR' && (
        <NoticeCard title="연소근로자 추가 안내">
          ...
          <p className="text-base leading-7 text-[var(--nexus-on-bg)]/70">
            친권자 동의서는 PDF에 함께 출력됩니다. 사업주는 출력 후 근로자에게 작성본을 받아
            보관하면 됩니다.
          </p>
        </NoticeCard>
      )}
      ...
      {form.contractType === 'PART_TIME' && (
        <NoticeCard title="단시간근로자 작성 안내">
          <p className="text-base leading-7 text-[var(--nexus-on-bg)]/70">
            단시간근로자의 요일별 근로시간 표는 빈칸으로 출력됩니다. 출력 후 실제 근무 요일과 시간은
            직접 작성하세요.
          </p>
        </NoticeCard>
      )}
...
function FormSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] shadow-sm">
      <CardHeader>
        <h2 className="text-2xl font-black text-[var(--nexus-primary)]">{title}</h2>
      </CardHeader>
...
function TextField({
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      <Label className="text-base font-black text-[var(--nexus-primary)]">{label}</Label>
...
function PhoneField({
  ...
  const [first = '010', second = '', third = ''] = value ? value.split('-') : ['010', '', ''];
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-[110px_1fr_1fr] gap-2">
        ...
        <Input
          ...
          inputMode="numeric"
          placeholder="1234"
          onChange={(e) => update(first, e.target.value.replace(/\D/g, ''), third)}
          className="h-12 border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] text-base"
        />
        ...
        <Input
          ...
          inputMode="numeric"
          placeholder="5678"
          onChange={(e) => update(first, second, e.target.value.replace(/\D/g, ''))}
          className="h-12 border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] text-base"
        />
      </div>
    </div>
