  ...
  return (
    <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] shadow-sm">
      <CardContent className="p-8">
        <h2 className="text-2xl font-black text-[var(--nexus-primary)]">계약서 유형 선택</h2>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          {contractTypes.map((type) => {
            ...
            return (
              ...
              >
                <p className="text-base font-black leading-snug break-keep">{type.title}</p>
                <p
                  className={`mt-3 text-sm leading-6 break-keep ${
                    active ? 'text-[var(--nexus-on-primary)]/80' : 'text-[var(--nexus-on-bg)]/55'
                  }`}
                >
                  {type.description}
                </p>
          ...
          {contractTypes.length % 2 !== 0 && <div aria-hidden="true" />}
        </div>
      </CardContent>
    </Card>
  );
