...
export function WorkerContractActions({ form }: { form: WorkerContractRequest }) {
  ...
  return (
    <Card className="h-fit border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] shadow-sm">
      <CardHeader>
        <p className="text-base font-black text-[var(--nexus-secondary)]">PDF 생성</p>

        <h2 className="text-3xl font-black text-[var(--nexus-primary)]">계약서 준비하기</h2>
      ...
      <CardContent>
        <div className="space-y-4 text-base">
          <Summary label="계약서 유형" value={CONTRACT_TYPE_LABEL[form.contractType]} />
          <Summary label="사업주" value={form.employerName || '-'} />
          <Summary label="근로자" value={form.workerName || '-'} />
          <Summary label="근무장소" value={form.workplace || '-'} />
          <Summary label="임금" value={form.wage ? `${form.wageType} ${form.wage}` : '-'} />
        </div>
