...
export function WorkerResultCard({ result }: { result: WorkerCalculateResponse | null }) {
  return (
    <Card className="h-fit border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] shadow-sm">
      <CardHeader>
        <h2 className="text-xl font-black text-[var(--nexus-primary)]">계산 결과</h2>
        <p className="text-sm text-[var(--nexus-outline)]">결과는 입력값 기준 참고용입니다.</p>
      </CardHeader>
  ...
  return (
    <div className="rounded-2xl bg-[var(--nexus-surface-lowest)] p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <Badge className="bg-[var(--nexus-primary)] text-[var(--nexus-on-primary)]">{badge}</Badge>
      </div>

      <p className="mt-4 text-2xl font-black text-[var(--nexus-on-bg)]">{value}</p>
      <p className="mt-2 text-sm leading-relaxed text-[var(--nexus-outline)]">{reason}</p>
    </div>
  );
}
