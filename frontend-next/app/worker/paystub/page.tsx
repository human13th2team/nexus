...
import { AlertTriangle, Building2, FileText, Phone, Printer } from 'lucide-react';
...
export default function PaystubPage() {
  ...
  const calculated = useMemo<PaystubCalculated>(() => {
    ...
    const grossPay =
      basePay + weeklyAllowancePay + overtimePay + nightPay + holidayPay + extraAllowance + bonus;
  ...
  return (
    <main className="min-h-screen bg-[var(--nexus-bg)] text-[var(--nexus-on-bg)]">
      <section className="mx-auto w-full max-w-[1120px] px-6 py-14">
        <div className="mb-10">
          <p className="text-sm font-black text-[var(--nexus-secondary)]">고용 가이드</p>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-[var(--nexus-outline)]">
            임금 지급 시 필요한 급여명세서를 작성하고 출력할 수 있습니다. 1차 버전에서는 4대보험과
            세금 공제액을 직접 입력합니다.
          </p>
        </div>
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-[var(--nexus-outline)]">
                  급여명세서에는 지급항목, 계산방법, 공제내역이 들어가야 합니다. 공제 금액이
                  정확하지 않으면 분쟁이 생길 수 있으므로 실제 보험료와 세금은 공단 또는 홈택스에서
                  확인하는 것을 권장합니다.
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          <aside className="h-fit space-y-6">
            <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-container)]">
              <CardHeader>
                <p className="text-sm font-black text-[var(--nexus-secondary)]">계산 결과</p>
                <h2 className="text-2xl font-black text-[var(--nexus-primary)]">지급 요약</h2>
              </CardHeader>

              <CardContent>
                <div className="space-y-3 text-sm">
                  <Summary label="총 지급액" value={formatWon(calculated.grossPay)} strong />
                  <Summary label="총 공제액" value={formatWon(calculated.totalDeduction)} />
                  <Separator className="my-4 bg-[var(--nexus-outline-variant)]" />
                  <Summary label="실지급액" value={formatWon(calculated.netPay)} strong />
                </div>
            ...
            <PaystubPreview refTarget={printRef} form={form} calculated={calculated} />
          </aside>
        </div>
      </section>
    </main>
    ...
    <div className="flex justify-between gap-4">
      ...
      <span
        className={`text-right ${
          strong ? 'text-lg font-black text-[var(--nexus-primary)]' : 'text-[var(--nexus-outline)]'
        }`}
      >
        {value}
      </span>
    ...
    <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)]">
      <CardContent className="p-6" ref={refTarget}>
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-[var(--nexus-primary)]">급여명세서</h2>
        </div>
        ...
        <PreviewTable
          ...
          rows={[
            ...
            [
              '주휴수당',
              `${form.hourlyWage || 0} × ${form.weeklyAllowanceHours || 0}시간`,
        ...
        <div className="mt-5 rounded-xl bg-[var(--nexus-surface-container)] p-4">
          <PreviewRow label="총 지급액" value={formatWon(calculated.grossPay)} />
          <PreviewRow label="총 공제액" value={formatWon(calculated.totalDeduction)} />
          <PreviewRow label="실지급액" value={formatWon(calculated.netPay)} />
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-container)]">
      <CardContent className="p-6">
        ...
        <p className="mt-3 text-sm leading-relaxed text-[var(--nexus-outline)]">
          4대보험과 세금은 근로자 상황에 따라 달라질 수 있습니다. 정확한 금액은 아래 기관에서 확인한
          뒤 입력하세요.
        </p>
...
function GuideItem({ title, desc, icon = false }: { title: string; desc: string; icon?: boolean }) {
  return (
    <div className="rounded-xl bg-[var(--nexus-surface-lowest)] p-4">
      ...
      <p className="mt-2 text-sm leading-relaxed text-[var(--nexus-outline)]">{desc}</p>
    </div>
  );
}
