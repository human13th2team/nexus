...
import { ArrowRight, ArrowUpRightIcon, FileText, ReceiptText, ShieldAlert } from 'lucide-react';
      ...
      <section className="mx-auto w-full max-w-[1160px] px-6 py-20">
        <div className="mb-12">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--nexus-on-bg)]/75">
            근로시간과 시급을 입력하면 주휴수당, 휴게시간, 4대보험 적용 여부를 확인하고,
            근로계약서와 급여명세서까지 바로 준비할 수 있습니다.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-start">
              ...
              <div className="flex-1">
                ...
                <p className="mt-4 max-w-4xl text-base leading-8 text-[var(--nexus-on-bg)]/75">
                  단 하루 근무하더라도 근로조건은 서면으로 명시하고 교부해야 하며, 임금을 지급할
                  때는 임금명세서도 함께 제공해야 합니다. 많은 사장님들이 놓치기 쉬운 부분이지만,
                  미작성 또는 미교부 시 과태료 대상이 될 수 있습니다.
                </p>
...
function SectionTitle({
  ...
  return (
    <div>
      ...
      <h2 className="text-4xl font-black leading-tight text-[var(--nexus-primary)]">{title}</h2>
    ...
    <div className="rounded-2xl border border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] p-6 transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-xl font-black text-[var(--nexus-primary)]">{title}</h3>
          <p className="mt-2 text-base leading-7 text-[var(--nexus-on-bg)]/70">{description}</p>
        </div>
      </div>
    ...
    <Card className="border-[var(--nexus-outline-variant)] bg-[var(--nexus-surface-lowest)] shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <CardContent className="flex h-full flex-col justify-between p-8">
        <div>
          ...
          <p className="text-sm font-black text-[var(--nexus-secondary)]">{category}</p>

          <h3 className="mt-2 text-2xl font-black leading-snug text-[var(--nexus-primary)]">
            {title}
          </h3>

          <p className="mt-4 text-base leading-7 text-[var(--nexus-on-bg)]/70">{description}</p>
        </div>
