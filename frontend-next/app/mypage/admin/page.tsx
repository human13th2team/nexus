...
export default function AdminPage() {
  ...
  const handleDeleteBoard = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/v1/mypage/admin/boards/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        alert('삭제되었습니다.');
  ...
  const handleDeleteComment = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/v1/mypage/admin/comments/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        alert('삭제되었습니다.');
  ...
  const handleToggleSuspension = async (id: string, currentStatus: boolean) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/v1/mypage/admin/users/${id}/suspend`, {
        method: 'PATCH',
      });
      if (res.ok) {
        alert(`${action}되었습니다.`);
