      ...
      <main className="max-w-6xl mx-auto px-6 py-16 space-y-16">
        ...
        <section className="bg-white p-12 rounded-[3.5rem] border border-[var(--nexus-outline-variant)]/30 shadow-[0_30px_60px_-15px_rgba(7,30,39,0.05)] flex flex-col md:flex-row gap-12 items-center relative overflow-hidden">
          ...
          <div className="w-48 h-48 bg-[var(--nexus-surface-low)] rounded-[3rem] flex items-center justify-center shrink-0 border border-[var(--nexus-outline-variant)]/20 shadow-inner">
            {brand.industryCategoryId === 'Tech' ? (
              <Rocket className="w-24 h-24 text-[var(--nexus-primary)]" />
            ) : (
              <Palette className="w-24 h-24 text-[var(--nexus-secondary)]" />
            )}
          </div>
          ...
          <div className="space-y-8 text-center md:text-left flex-1 relative">
            <div className="space-y-3">
              ...
              <h2 className="text-5xl font-black tracking-tight text-[var(--nexus-on-bg)]">
                {brand.title}
              </h2>
            </div>

            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
              {brand.keywords?.extracted_keywords &&
                brand.keywords.extracted_keywords.map((kw: string, i: number) => (
                  <span
                    key={i}
                    className="px-5 py-2 bg-[var(--nexus-surface-low)] text-[var(--nexus-primary)] text-[10px] font-black rounded-full uppercase tracking-wider border border-[var(--nexus-outline-variant)]/20"
                  >
                    #{kw}
                  </span>
                ))}
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-12">
                    ...
                    <div className="text-right">
                      <span className="px-4 py-1.5 bg-[var(--nexus-primary)] text-white text-[10px] font-black rounded-full uppercase tracking-[0.2em] shadow-lg shadow-[var(--nexus-primary)]/20">
                        Final Selection
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col lg:flex-row gap-16">
                    <div className="flex-1 space-y-10">
                      <div className="space-y-2">
                        <h4 className="text-5xl font-black text-[var(--nexus-on-bg)] leading-tight">
                          {identity.brandName}
                        </h4>
                        <p className="text-2xl font-bold italic text-[var(--nexus-secondary)] opacity-60">
                          "{identity.slogan}"
                        </p>
                      </div>
                  ...
                  <div key={asset.id} className="group flex flex-col gap-6">
                        ...
                        <img
                          src={
                            asset.fileUrl.startsWith('http')
                              ? asset.fileUrl
                              : `${FASTAPI_BASE_URL}${asset.fileUrl}`
                          }
                          alt={asset.type}
                    ...
                    <div className="px-6">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <button
                          onClick={() => {
                            const url = asset.fileUrl.startsWith('http')
                              ? asset.fileUrl
                              : `${FASTAPI_BASE_URL}${asset.fileUrl}`;
