  ...
  if (!namingOptions || namingOptions.length === 0) {
    return (
      <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
        ...
        <button onClick={onBack} className="mt-4 text-sm font-bold text-black underline">
          돌아가기
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center space-y-3">
        <h2 className="text-3xl font-black text-[var(--nexus-on-bg)] tracking-tight">
          Identity Candidates
        </h2>
        <p className="text-gray-500 font-medium max-w-lg mx-auto text-sm">
          대표님의 비즈니스 모델에 가장 부합하는 3가지 브랜드 정체성 후보입니다.
        </p>
      </div>
          ...
          >
            ...
            <div className="space-y-6 relative">
              <div className="space-y-2">
                <h3 className="font-black text-2xl text-[var(--nexus-on-bg)] group-hover:text-[var(--nexus-primary)] transition-colors">
                  {item.brandName}
                </h3>
                <p className="text-sm text-[var(--nexus-secondary)] font-bold italic opacity-70 leading-relaxed">
                  "{item.slogan}"
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {item.keywords?.map((kw) => (
                  <span
                    key={kw}
                    className="text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1.5 bg-[var(--nexus-surface-container)] text-[var(--nexus-primary)] rounded-lg"
                  >
                    #{kw}
                  </span>
                ))}
